...
import argparse
import os
from data_utils.DDateLoader import ISPRSDataset
import torch
import datetime
import logging
...
def main(args):
    ...
    TRAIN_DATASET = ISPRSDataset(split='train', data_root=root, num_point=NUM_POINT, block_size=args.block_size, stride=stride, padding=padding, transform=None)
    print("start loading test data ...")
    TEST_DATASET = ISPRSDataset(split='test', data_root=root, num_point=NUM_POINT, block_size=args.block_size, stride=stride, padding=padding, transform=None)
        ...
        with torch.no_grad():
            ...
            for l in range(NUM_CLASSES):
                iou_per_class_str += 'class %s weight: %.3f, IoU: %.3f , Acc: %.3f \n' % (
                    ...
                    total_correct_class[l] / float(total_iou_deno_class[l]), acc_list[l])
            ...
            if mIoU >= best_iou:
                ...
                savepath = str(checkpoints_dir) + '/best_iou_model.pth'
                log_string('Saving at %s' % savepath)
                state = {
                    'epoch': epoch,
                    'class_avg_iou': mIoU,
